  ...
  useEffect(() => {
    const swipeGesture = () => {
      if (touch.current.end.x - touch.current.start.x < -50) {
        const prevIndex = currentSlider !== 0 ? currentSlider - 1 : componentProps.sliders.length - 1

        setCurrentSlider(prevIndex)
      }

      if (touch.current.end.x - touch.current.start.x > 50) {
        const nextIndex = currentSlider !== componentProps.sliders.length - 1 ? currentSlider + 1 : 0
      ...
      <div id={styles["SliderElementContainer"]}>
        ...
        >
          {componentProps.sliders.map((slider, index) => (
            <div
              className={`${styles["ImageElementContainer"]}${currentSlider === index ? ` ${styles["Active"]}` : ""}`}
            ...
            >
              ...
              <div className={styles["ImageDescriptionElement"]}>
                <h2 ref={(element) => (imageElementDescriptionTitles.current[index] = element as HTMLHeadingElement)}>
                  {slider.title.replace(/<nbs>/g, "\u00A0")}
                </h2>
                <p ref={(element) => (imageElementDescriptionTexts.current[index] = element as HTMLParagraphElement)}>
              ...
              <div
                ...
                ref={(element) => (imageForegroundElementWrappers.current[index] = element as HTMLDivElement)}
              >
                <img
      ...
      {!isMobile && (
        <div id={styles["AdvancePillsWrapper"]}>
          {componentProps.sliders.map((_, index) => (
            <div
              className={`${styles["AdvancePillElement"]}${currentSlider === index ? ` ${styles["Active"]}` : ""}`}
