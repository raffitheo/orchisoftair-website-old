...
const Navbar: React.FC<NavbarProps> = (componentProps: NavbarProps) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (desktopContacts && desktopContacts.current && navbarWrapper && navbarWrapper.current) {
              ...
              <div id={styles["MobileHamburgerContainer"]}>
                <div id={styles["MobileHamburger"]}>
                  <input
                    ...
                    onChange={(event) => {
                      componentProps.onMobileMenuChange(!isMobileMenuOpen, event)
                  ...
                  {componentProps.navigation.map((element, elementIndex) => (
                    <li
                      className={`${styles["NavigationListElement"]}${
                        currentlySelected === elementIndex ? ` ${styles["Active"]}` : ""
                      }`}
                            ...
                            {element.subMenu.map((subMenuElement, subMenuElementIndex) => (
                              <li
                                className={`${styles["NavigationSubMenuListElement"]}${
                                  currentlySelected === elementIndex && currentlySelectedSubMenu === subMenuElementIndex
                                    ? ` ${styles["Active"]}`
                                    : ""
                                }`}
